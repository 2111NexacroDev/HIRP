...
public class TimeStoreLogic implements TimeStore{
	...
	public Time selectTime(SqlSession sqlSession, String emplId) {
		Time time = sqlSession.selectOne("TimeMapper.selectTime", emplId);
		return time;
	}

	// 사용자 출퇴근 내역 조회
	@Override
	public List<Time> selectWorkView(SqlSession sqlSession, String emplId) {
		List<Time> time = sqlSession.selectList("TimeMapper.selectWorkView", emplId);
		return time;
	}
	...
	// 사용자 근태 조정 조회
	@Override
	public List<TimeModify> selectModifyView(SqlSession sqlSession, String emplId) {
		List<TimeModify> modify = sqlSession.selectList("TimeMapper.selectModifyView", emplId);
		return modify;
	}
